...
import { getActiveCourses, getPageContent } from "@/lib/firebase/firestore";
...
export default async function CoursesPage() {
  const [page, activeCourses] = await Promise.all([
    getPageContent("egitim-programi"),
    getActiveCourses(),
  ]);
  const featuredCourse =
    activeCourses.find((course) => course.slug === "tyt-ayt") ?? activeCourses[0];

  return (
    <>
      <PageHero
        page={page}
        fallbackTitle="Eğitimlerimiz"
                ...
                <div>
                  ...
                  <div className="overflow-hidden rounded-xl border border-outline/10">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {featuredCourse.scheduleTable.map((row, index) => (
                          <tr
                            key={row.day}
                            className={
                              index % 2 === 0 ? "bg-white" : "bg-surface-gray/50"
                            }
                          >
